...
(function(){
  ...
  function fd(v){
    const m=String(v||'').match(/(19|20)\d{2}-\d{2}-\d{2}/);
    if(!m)return v||'-';
  ...
  function downloadRbiExcel(){
    ...
    const meta=[
      ...
      ['Assessment Period','24 Months (1AP)'],
      ['Wilayah Kerja',region],
      ['Location',location],
      ['Search',search||'-'],
      ['Total Asset',list.length],
      []
    ];
    const matrixRows=[['Likelihood / Consequence','A','B','C','D','E']];
    [5,4,3,2,1].forEach(r=>matrixRows.push([r,...['A','B','C','D','E'].map(c=>{
      const k=r+c; return list.filter(a=>String(a.risk1AP||a.risk||'').toUpperCase()===k).length;
    })]));
    meta.push(['RISK MATRIX 1AP'],...matrixRows);
    const wsMatrix=XLSX.utils.aoa_to_sheet(meta);
    wsMatrix['!cols']=[{wch:30},{wch:24},{wch:18},{wch:18},{wch:18},{wch:18}];
    wsMatrix['!merges']=[{s:{r:0,c:0},e:{r:0,c:1}}];
    XLSX.utils.book_append_sheet(wb,wsMatrix,'RBI Summary');

    const categories={F:'Favourable',A:'Acceptable',T:'Tolerable',U:'Unsatisfactory',C:'Critical',N:'No AP Detected'};
    const catRows=[['Kategori','Jumlah','Persentase']];
    const catMap={F:0,A:0,T:0,U:0,C:0,N:0};
    ...
    const wsSummary=XLSX.utils.aoa_to_sheet([['RBI RISK SUMMARY'],[],...catRows]);
    wsSummary['!cols']=[{wch:24},{wch:14},{wch:16}];
    ...
    const wsDue=XLSX.utils.aoa_to_sheet([['RBI INSPECTION DUE PLANNING'],[],...dueRows]);
    wsDue['!cols']=[{wch:7},{wch:18},{wch:28},{wch:20},{wch:24},{wch:12},{wch:20},{wch:18},{wch:20}];
    wsDue['!autofilter']={ref:`A3:I${dueRows.length+2}`};
    XLSX.utils.book_append_sheet(wb,wsDue,'Inspection Due');

    const wbout=XLSX.write(wb,{bookType:'xlsx',type:'array'});
    const blob=new Blob([wbout],{type:'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet'});
    const url=URL.createObjectURL(blob); const a=document.createElement('a');
    const stamp=new Date().toISOString().slice(0,10);
    a.href=url;a.download=`RBI_Assessment_${stamp}.xlsx`;document.body.appendChild(a);a.click();a.remove();URL.revokeObjectURL(url);
  }
  window.downloadRbiExcel=downloadRbiExcel;
  document.addEventListener('DOMContentLoaded',function(){
    const b=document.getElementById('rbiExport');
    if(b){b.textContent='⇩  Export Excel';b.onclick=downloadRbiExcel;}
  });
})();
